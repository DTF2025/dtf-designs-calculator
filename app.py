...
def calculate_area_pricing(form_data):
    ...
    try:
        ...
        if category == 'Decals':
            # NEW 2025 DECAL PRICING SYSTEM
            # Build form data for new pricing system
            decal_form_data = {
                "width_in": float(form_data.get("width", width_in)),
                "height_in": float(form_data.get("height", height_in)),
                "qty": int(form_data.get("quantity", qty)),
            }
            
            # MATERIAL
            raw_mat = (form_data.get("vinyl_material") or "").lower()
            decal_form_data["material"] = "matte" if "matte" in raw_mat else "gloss"

            # CUT TYPE
            raw_cut = (form_data.get("cut_type") or form_data.get("cut") or "").lower()
            decal_form_data["cut_type"] = "die" if "die" in raw_cut else "kiss"

            # LAMINATE - check all possible field names
            raw_lam = (form_data.get("lam") or form_data.get("laminate") or form_data.get("laminate_name") or form_data.get("protective_laminate") or "none").lower()
            if "gloss" in raw_lam: 
                decal_form_data["laminate"] = "gloss"
            elif "matte" in raw_lam: 
                decal_form_data["laminate"] = "matte"
            elif "cast" in raw_lam: 
                decal_form_data["laminate"] = "uv"
            else: 
                decal_form_data["laminate"] = "none"

            # Get retail quote and true cost
            retail_result = calculate_decal_retail_price(
                decal_form_data["width_in"], 
                decal_form_data["height_in"], 
                decal_form_data["qty"],
                decal_form_data.get("material", "gloss"),
                decal_form_data.get("laminate", "none")
            )
            cost_result = calculate_decal_true_cost(
                decal_form_data["width_in"], 
                decal_form_data["height_in"], 
                decal_form_data["qty"],
                decal_form_data.get("material", "gloss"),
            ...
            ret_unit = retail_result["unit_usd"]
            ret_total = retail_result["total_usd"]
            cost_unit = cost_result["unit_cost_usd"]
            cost_total = cost_result["total_cost_usd"]

            # Apply customer type pricing
            if customer_type == 'partner':
                unit_price = ret_unit * 0.70  # 30% partner discount
            elif customer_type == 'employee':
                unit_price = cost_unit  # Employee sees true cost
            else:
                unit_price = ret_unit  # Retail customer
            
            quoted_price = unit_price * decal_form_data["qty"]
            
            # Return decal pricing immediately (don't fall through to old system)
            return {
                'totals': {
                    'your_cost': round(cost_total, 2),  # Use true cost, not old cost
                    'quoted_price': round(quoted_price, 2),
                    'unit_price': round(unit_price, 2)
                },
                'costs': {
                    'decal_retail_unit': round(ret_unit, 2),
                    'decal_retail_total': round(ret_total, 2),
                    'decal_cost_unit': round(cost_unit, 2),
                    'decal_cost_total': round(cost_total, 2)
                },
                'derived': {
                    'billable_sqft': round(billable_sqft, 2),
                    'decal_material': decal_form_data["material"],
                    'decal_cut': decal_form_data["cut_type"],
                    'decal_lam': decal_form_data["laminate"]
                }
            }
            ...
            area_sqft = (width_in/12.0) * (height_in/12.0)  # or width_ft*height_ft if you already have feet
...
def admin_required(f):
    """Decorator to require admin authentication - DISABLED FOR TESTING"""
...
def partner_required(f):
    """Decorator to require partner authentication - DISABLED FOR TESTING"""
    def decorated(*args, **kwargs):
        # Testing mode: bypass authentication
        return f(*args, **kwargs)
    decorated.__name__ = f.__name__
...
def admin_login():
    """Admin login - DISABLED: redirect directly to admin"""
...
def partner_login():
    """Partner login - DISABLED: redirect directly to partner calculator"""
...
def employee_login():
    """Employee login - DISABLED: redirect directly to employee calculator"""
        ...
        if request.method == 'POST':
            try:
                ...
                if selected_category == 'Banner':
                    ...
                    # Map form media_name to material code
                    media_selection = form_data.get('media_name', '')
                    if 'jetflex' in media_selection.lower():
                        material = 'jetflex'
                    else:
                        material = 'alpha'  # Default to alpha
                        
                    # ---- sides normalization
                    raw_sides = (form_data.get("print_sides")
                                 or form_data.get("sides")
                                 or form_data.get("double_sided")
                                 or "").strip().lower()

                    double_sided = (
                        raw_sides in {"double", "double-sided", "double_sided", "2", "true", "on", True}
                    )

                    # ---- rush normalization
                    raw_rush = str(form_data.get("rush", "")).strip().lower()
                    rush = raw_rush in {"true", "on", "rush", "1", "yes"}

                    # build addons dict
                    addons = {
                        "reinforced_corners": form_data.get('hem_opt', 'None') != 'None',
                        "wind_slits": False,
                        "pole_pockets_pairs": int(form_data.get('grommets', 0)) // 2,
                    ...
                    employee_cost_result = banner_employee_cost(
                        ...
                        qty=qty,
                        material=material,
                        reinforced_corners=addons["reinforced_corners"],
                        wind_slits=addons["wind_slits"],
                        pole_pockets_pairs=addons["pole_pockets_pairs"],
                        double_sided=addons["double_sided"]
                    )
                    
                    # Calculate retail pricing for Customer Quote panel
                    from scripts.pricers import banner_price_retail, banner_cost_true
                    
                    # RETAIL (for Customer Quote panel) - includes waste
                    retail_unit, retail_total = banner_price_retail(
                        width_ft, height_ft, qty,
                        material=material,
                        include_waste=True,
                        reinforced_corners=addons["reinforced_corners"],
                        wind_slits=addons["wind_slits"],
                    ...
                    # TRUE COST (for Employee Analysis)
                    cost_unit, cost_total = banner_cost_true(
                        width_ft, height_ft, qty,
                        material=material,
                        reinforced_corners=addons["reinforced_corners"],
                        wind_slits=addons["wind_slits"],
                    ...
                    # Billable area shown to customer (includes +1 sqft waste)
                    billable_area_sqft = (width_ft * height_ft) + 1.0
                    employee_area_sqft = width_ft * height_ft * qty
                    
                    result = {
                        'totals': {
                            'quoted_price': employee_cost_result['total_cost_usd'],
                            'unit_price': employee_cost_result['unit_cost_usd'], 
                            'total_sqft': employee_area_sqft
                        },
                        'breakdown': {
                        ...
                        'derived': {
                            'billable_sqft': round(employee_area_sqft, 2)
                        },
                        'customer_quote': {
                            'area_sqft': round(billable_area_sqft, 2),
                            'unit_price': round(retail_unit, 2),
                            'total_price': round(retail_total, 2),
                        }
                    }
                ...
                if result and 'totals' in result:
                    try:
                        ...
                        if category == 'Banner':
                            # Use already calculated retail values that include double-sided pricing
                            true_cost = cost_total  # Use true cost from banner_cost_true 
                            retail_price = retail_total  # Use retail from banner_price_retail
                            
                        elif category == 'Decals':
                            # Use NEW 2025 decal pricing system
                            width_in = float(form_data.get('width_in', 4))
                            height_in = float(form_data.get('height_in', 4))  
                            qty = int(form_data.get('qty', 1))
                            
                            # Build decal form data
                            decal_form_data = {
                                "width_in": width_in,
                                "height_in": height_in,
                                "qty": qty,
                            ...
                            # Material normalization (same as interface)
                            raw_mat = (form_data.get("vinyl_material") or form_data.get("media_name") or "").lower()
                            decal_form_data["material"] = "matte" if "matte" in raw_mat else "gloss"
                            
                            # Cut type normalization
                            raw_cut = (form_data.get("cut_type") or form_data.get("cut") or "").lower()
                            decal_form_data["cut_type"] = "die" if "die" in raw_cut else "kiss"
                            
                            # Laminate normalization - check all possible field names including "lam"
                            raw_lam = (form_data.get("lam") or form_data.get("laminate_name") or form_data.get("laminate") or "none").lower()
                            if "gloss" in raw_lam: 
                                decal_form_data["laminate"] = "gloss"
                            elif "matte" in raw_lam: 
                                decal_form_data["laminate"] = "matte"
                            ...
                            true_cost = cost_result["total_cost_usd"]
                            retail_price = retail_result["total_usd"]
                        ...
                        # Partner pricing - adjusted for laminated products to maintain consistent margins
                        if category == 'Decals' and 'lam' in form_data and form_data.get('lam') != 'none':
                            # For laminated decals, use less aggressive discount to maintain profitability
                            discount_partner_price = retail_price * 0.75  # 25% discount instead of 30%
                        else:
                            discount_partner_price = retail_price * 0.7   # Standard 30% discount
                        ...
                        result['employee_analysis'] = {
                            ...
                            'retail_unit': round(retail_unit, 2) if category == 'Banner' else round(retail_price / qty, 2),
                            'retail_total': round(retail_price, 2),
                            'partner_unit': round((retail_unit if category == 'Banner' else retail_price / qty) * 0.70, 2),
                            'partner_total': round(retail_price * 0.70, 2),
                            ...
                            'vs_industry_low': round(abs(retail_price - industry_low), 2),
                            'vs_industry_high': round(abs(retail_price - industry_high), 2)
                        }
                        
                        # CRITICAL: For banners, preserve the correct $11.00 values - DO NOT OVERRIDE!
                        # For other categories, update totals with true cost
                        if category != 'Banner':
                            result['totals']['quoted_price'] = round(true_cost, 2)
                            result['totals']['unit_price'] = round(true_cost / qty, 2) if qty > 0 else 0
                        else:
                            # For banners, ensure we preserve the correct banner_employee_cost() results
                            # The True Cost should already be correct at $11.00 from banner_employee_cost()
                            pass
                        ...
                        if form_data.get('category', 'Banner') != 'Banner':
                            result['employee_analysis'] = {
                                ...
                                'vs_industry_low': 0.0,
                                'vs_industry_high': round(abs(-result['totals']['quoted_price'] * 0.7), 2)
                            }
                        else:
...
def _require_admin():
    """Require admin token for protected endpoints - DISABLED FOR TESTING"""
    # Testing mode: bypass authentication
    pass

# ========== RETAIL QUOTE API ENDPOINTS ==========

@app.route("/quote/decal", methods=['POST'])
def quote_decal():
    """
    Retail decal quote with margin guard
    Body: {
        "width_in": float, "height_in": float, "qty": int,
        "material": str, "cut_type": str, "laminate": str
    }
    Response: {
...
def download_project():
    """Download the complete DTF Designs project as a compressed archive"""
    import os
    file_path = 'dtf-designs-clean.tar.gz'
    if os.path.exists(file_path):
        return send_file(file_path, as_attachment=True, download_name='dtf-designs-project.tar.gz')
    else:
        flash('Project archive not found. Please contact support.', 'error')
